...
class Camera(db.Model):
    ...
    detection_logs = relationship('DetectionLog', backref='camera', lazy=True)
...
class DetectionLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    camera_id = db.Column(db.Integer, ForeignKey('cameras.id'), nullable=False)
    emotion = db.Column(db.String(50), nullable=False)
    confidence = db.Column(db.Float, nullable=True)
    image_path = db.Column(db.String(255), nullable=True)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    def __init__(self, camera_id, emotion, timestamp=None, image_path=None, confidence=None):
    ...
    def __repr__(self):
        return f"<Log Cam#{self.camera_id} - {self.emotion} @ {self.timestamp}>"